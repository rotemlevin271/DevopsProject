...
def add_user(username, user_id):
    ...
    if not cursor.execute(f"SELECT user_name FROM {schema_name}.usersRotem WHERE id = {user_id}"):
        # Inserting data into table
        cursor.execute(f"INSERT into {schema_name}.usersRotem (user_id, user_name, creation_date) VALUES ('{username}', {user_id}, {user_creation_time})")
    else:
        cursor.close()
        conn.close()
        return 1
...
def del_user(user_id):
    ...
    if cursor.execute(f"SELECT user_name FROM {schema_name}.usersRotem WHERE id = {user_id}"):
...
def update_user(new_name, user_id):
    ...
    if cursor.execute(f"SELECT user_name FROM {schema_name}.usersRotem WHERE id = {user_id}"):
        ...
        cursor.execute(f"UPDATE {schema_name}.usersRotem SET user_name = '{new_name}' WHERE id = {user_id}")
    else:
        # Return error message if user_id doesn't exist
        cursor.close()
        conn.close()
...
def get_user(user_id):
    ...
    cursor.execute(f"SELECT user_name FROM usersRotem WHERE id = {user_id}")
    try:
